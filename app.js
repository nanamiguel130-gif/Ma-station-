...
function renderFuelFields(){
  ...
  box.innerHTML = labeled.map(fuel => `
    <div class="fuel">
      ...
      <div class="pump">
        ...
        <div class="full"><label>Prix / litre (FCFA)</label><input class="input" inputmode="decimal" id="fuel_${fuel.id}_price" value="${fuel.price || ''}"></div>
      </div>
    </div>
  `).join('') + `
  ...
  pompisteState.fuels.forEach(fuel => {
    ...
    const price = document.getElementById('fuel_' + fuel.id + '_price');
    ...
    if(price) price.onchange = () => persistPompiste();
  });
}

async function addFuelPump(type){
  pompisteState.fuels.push({ id: uid(), type, opening:'', closing:'', price:0 });
  renderFuelFields();
  updateServiceSummary();
...
function computeAndShowSituation(readOnly = false){
  ...
  if(creditsBox) creditsBox.innerHTML = renderClientCreditsReadOnly(pompisteState.clientCredits);
