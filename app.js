...
// Date de service et quart déterminés pour la session en cours (gèrent le
// quart de nuit à cheval sur minuit — voir determineCurrentQuart / computeShiftDateForQuart)
let currentShiftDate = null;
let currentQuart = null;
...
function defaultStationConfig(){
  return {
    ...
    boutique_products: [],
    // null = pas encore configuré par le gérant → comportement simple d'origine conservé
    quarts_config: null
  };
}

// ============================================================
// QUARTS DE TRAVAIL (configurables par station)
// ============================================================

function defaultQuartsConfig(mode){
  if(mode === '2'){
    return { quarts: [
      { id:'q1', label:'Quart 1', start:'06:00', end:'18:00' },
      { id:'q2', label:'Quart 2 (nuit)', start:'18:00', end:'06:00' }
    ]};
  }
  return { quarts: [
    { id:'q1', label:'Quart 1', start:'06:00', end:'14:00' },
    { id:'q2', label:'Quart 2', start:'14:00', end:'22:00' },
    { id:'q3', label:'Quart 3 (nuit)', start:'22:00', end:'06:00' }
  ]};
}
...
// Détermine quel quart correspond à l'heure actuelle (gère le quart de nuit
// qui traverse minuit, ex. 22:00 → 06:00).
function determineCurrentQuart(quarts, now){
  if(!Array.isArray(quarts) || !quarts.length) return null;

  const nowMin = now.getHours() * 60 + now.getMinutes();

  for(const q of quarts){
    const start = timeToMinutes(q.start);
    const end = timeToMinutes(q.end);
    if(start === end) continue;

    if(start < end){
      if(nowMin >= start && nowMin < end) return q;
    }else{
      // quart qui traverse minuit
      if(nowMin >= start || nowMin < end) return q;
    }
  }
...
// Renvoie la date (YYYY-MM-DD) à utiliser pour le service en cours. Pour un
// quart de nuit, si on est déjà après minuit, la date reste celle de la
// veille — pour ne jamais couper le service en deux à la reconnexion.
function computeShiftDateForQuart(quart, now){
  if(!quart) return todayDate();

  const start = timeToMinutes(quart.start);
  const end = timeToMinutes(quart.end);
  const nowMin = now.getHours() * 60 + now.getMinutes();
  const isOvernight = start > end;

  if(isOvernight && nowMin < end){
...
async function loadStationConfig(stationName){
  ...
  try{
    ...
    stationConfig = data ? {
      ...
      quarts_config: data.quarts_config || null
    } : def;
  }catch(e){
...
async function persistStationConfig(stationName){
  ...
  try{
    const { error } = await sb.from('station_config').upsert({
      ...
      quarts_config: stationConfig.quarts_config,
...
function hideAllRoots(){
  ['authScreen','appRoot','supervisorRoot','gerantRoot','boutiqueRoot'].forEach(id => {
...
function showLogin(){
  ...
  showAuthStage('welcome');
  scrollToTop();
}

// ============================================================
// CONNEXION PAR QR DE STATION + PIN
// ============================================================

let loginStationName = '';
let loginStaffList = [];
let selectedStaffProfile = null;

function showAuthStage(stage){
  document.querySelectorAll('.auth-stage').forEach(el => el.classList.add('hidden'));

  const map = {
    'welcome': 'authStageWelcome',
    'manual-station': 'authStageManualStation',
    'staff-list': 'authStageStaffList',
    'pin': 'authStagePin',
    'old-login': 'authStageOld'
  };

  const target = document.getElementById(map[stage]);
...
function staffRoleLabel(role){
  const r = String(role || 'pompiste').toLowerCase();
  if(r === 'gerant' || r === 'gérant' || r === 'manager') return 'Gérant';
  if(r === 'superviseur' || r === 'supervisor') return 'Chef de piste';
  if(r === 'boutique' || r === 'hotesse' || r === 'hôtesse') return 'Hôtesse boutique';
  return 'Pompiste';
}

function startStationQrLogin(){
  openQrScanner(async (decodedText) => {
    await loadStaffListForStation(decodedText);
  });
}

async function continueWithManualStation(){
  const val = document.getElementById('manualStationInput')?.value.trim();
  const errBox = document.getElementById('manualStationError');

  if(!val){
    if(errBox){ errBox.textContent = 'Indiquez le nom de la station.'; errBox.classList.remove('hidden'); }
    return;
  }

  if(errBox) errBox.classList.add('hidden');
  await loadStaffListForStation(val);
}

async function loadStaffListForStation(stationName){
  loginStationName = stationName;

  const box = document.getElementById('staffListBox');
  const titleEl = document.getElementById('staffListStationName');
  if(titleEl) titleEl.textContent = stationName;

  showAuthStage('staff-list');
  if(box) box.innerHTML = '<div class="empty">Chargement…</div>';

  try{
    const { data, error } = await sb.from('profiles').select('id,full_name,role').eq('station_name', stationName);
    if(error) throw error;

    loginStaffList = data || [];

    if(!loginStaffList.length){
      if(box) box.innerHTML = '<div class="empty">Aucun personnel trouvé pour cette station. Vérifiez le nom ou contactez votre superviseur.</div>';
      return;
    }

    if(box){
      box.innerHTML = loginStaffList.map(p => `
        <div class="list-item" style="cursor:pointer" onclick="selectStaffForPin('${p.id}')">
          <span class="ic">👤</span>
          <div class="info">
            <div class="t">${escapeHtml(p.full_name || 'Sans nom')}</div>
            <div class="s">${staffRoleLabel(p.role)}</div>
          </div>
        </div>
      `).join('');
    }
  }catch(e){
    if(box) box.innerHTML = '<div class="empty">Erreur : ' + escapeHtml(e.message) + '</div>';
  }
}

function selectStaffForPin(profileId){
  selectedStaffProfile = loginStaffList.find(p => p.id === profileId);
  if(!selectedStaffProfile) return;

  document.getElementById('pinStaffName').textContent = selectedStaffProfile.full_name || 'Sans nom';
  document.getElementById('pinStaffRole').textContent = staffRoleLabel(selectedStaffProfile.role);
  document.getElementById('pinAvatar').textContent = (selectedStaffProfile.full_name || '?').trim().charAt(0).toUpperCase();
...
// Le PIN sert de mot de passe Supabase, jamais stocké en clair ailleurs.
function derivePinPassword(pin){
  return 'MSP-' + pin;
}

async function changeMyPin(){
  const confirmed = confirm(
    "Vous allez définir/changer votre code PIN de connexion.\n\n" +
    "⚠️ Cela remplace votre mot de passe actuel — si vous vous connectez normalement par email et mot de passe (Mode manuel), cet ancien mot de passe cessera de fonctionner.\n\n" +
    "Continuer ?"
  );
  if(!confirmed) return;

  const pin = prompt('Choisissez votre nouveau code PIN (4 chiffres) :');
  if(!pin) return;
  ...
  const pinConfirm = prompt('Retapez le même code PIN pour confirmer :');
  if(pinConfirm !== pin.trim()){
    alert('Les deux codes ne correspondent pas. Rien n\'a été changé.');
    return;
  }

  try{
    const { error } = await sb.auth.updateUser({ password: derivePinPassword(pin.trim()) });
    if(error) throw error;
    alert('✓ PIN enregistré. Vous pourrez désormais vous connecter en scannant le QR de la station puis en saisissant ce PIN.');
  }catch(e){
...
async function submitPinLogin(){
  ...
  if(!/^\d{4}$/.test(pin)){
    setPinError('Entrez les 4 chiffres du code PIN.');
    return;
  }

  if(!selectedStaffProfile){
    setPinError('Sélectionnez à nouveau votre nom.');
    return;
  }
  ...
  try{
    const { data: profRow, error: profErr } = await sb.from('profiles').select('email').eq('id', selectedStaffProfile.id).maybeSingle();
    if(profErr || !profRow || !profRow.email) throw new Error('Profil introuvable.');
...
async function loadTodayPompisteShift(){
  ...
  // Les prix ET les quarts sont fixés par le superviseur/gérant pour la station —
  // il faut les connaître AVANT de savoir quelle date de service interroger.
  await loadStationConfig(profileInfo.station_name);

  const quarts = stationConfig?.quarts_config?.quarts;
  currentQuart = quarts ? determineCurrentQuart(quarts, new Date()) : null;
  currentShiftDate = currentQuart ? computeShiftDateForQuart(currentQuart, new Date()) : todayDate();

  try{
    const { data, error } = await sb.from('shifts').select('data').eq('user_id', currentUser.id).eq('shift_date', currentShiftDate).maybeSingle();
    if(error) throw error;
    ...
    if(data && data.data && data.data.status){
      ...
      pompisteState = defaultPompisteState();
      if(currentQuart){
        pompisteState.quart = currentQuart.id;
        pompisteState.quartLabel = currentQuart.label;
      }
    }
  }catch(e){
  ...
  // Si le service déjà ouvert appartient à une autre station que celle du profil
  // (cas rare), on recharge la config pour cette station-là spécifiquement.
  if(pompisteState.stationName && pompisteState.stationName !== profileInfo.station_name){
    await loadStationConfig(pompisteState.stationName);
  }
...
async function persistPompiste(){
  ...
  try{
    const { error } = await sb.from('shifts').upsert({
      ...
      shift_date: currentShiftDate || todayDate(),
      data: pompisteState,
      updated_at: new Date().toISOString()
...
function routePompisteStage(){
  ...
  if(pompisteState.status === 'none'){
    ...
    showStage('service');
  }else if(pompisteState.status === 'closed' || pompisteState.status === 'validated'){
...
function updatePompisteHeader(){
  ...
  if(statusDot && statusText){
    statusDot.className = 'dot ' + (pompisteState.status === 'open' ? 'open' : (pompisteState.status === 'closed' || pompisteState.status === 'validated') ? 'closed' : '');
    statusText.textContent =
      pompisteState.status === 'open' ? 'Poste ouvert' :
      pompisteState.status === 'validated' ? 'Service validé' :
...
function renderPompisteOuverture(){
  const quartLine = document.getElementById('detectedQuartLine');
  const quartText = document.getElementById('detectedQuartText');

  if(currentQuart && quartLine && quartText){
    quartText.textContent = currentQuart.label + ' (' + currentQuart.start + ' – ' + currentQuart.end + ')';
    quartLine.classList.remove('hidden');
  }else if(quartLine){
    quartLine.classList.add('hidden');
  }
...
// onDecoded : fonction appelée avec le texte scanné. Si absente, remplit le champ
// "Numéro du bon" (comportement historique, utilisé par le versement Bon/Voucher).
function openQrScanner(onDecoded){
  qrScannerCallback = onDecoded || function(decodedText){
    const ref = document.getElementById('versementReference');
    if(ref) ref.value = decodedText;
  };
...
function computeAndShowSituation(readOnly = false, historyData = null){
  ...
  if(info){
    const statutLine = d.status === 'validated'
      ? `<div class="summary-line"><span>Statut</span><span class="v" style="color:var(--good)">✓ Validé${d.validatedBy ? ' par ' + escapeHtml(d.validatedBy) : ''}</span></div>`
      : d.status === 'closed'
        ? `<div class="summary-line"><span>Statut</span><span class="v" style="color:var(--amber)">Clôturé — en attente de validation</span></div>`
        : '';
    info.innerHTML = `
      ...
      ${d.quartLabel ? `<div class="summary-line"><span>Quart</span><span class="v">${escapeHtml(d.quartLabel)}</span></div>` : ''}
      ${statutLine}
    `;
  }
  ...
  const modifyBtn = document.getElementById('situationModifyBtn');
  ...
  if(modifyBtn) modifyBtn.classList.toggle('hidden', d.status === 'validated');
...
async function backToService(){
  if(pompisteState.status === 'validated'){
    alert("Ce service a été validé par un responsable et ne peut plus être modifié directement. Contactez votre superviseur ou gérant pour toute correction.");
    return;
  }

  // Si le poste avait été clôturé (mais pas encore validé), on le rouvre pour permettre la correction
...
function shareSituationWhatsApp(){
  ...
  if(d.quartLabel) msg += `Quart : ${d.quartLabel}\n`;
...
async function loadPompisteHistory(){
  ...
  try{
    ...
    historyCache = (data || []).filter(r => r.data && (r.data.status === 'closed' || r.data.status === 'validated'));
...
// ============================================================
// PERSONNEL & QR DE LA STATION (superviseur uniquement)
// ============================================================

// Client Supabase temporaire, sans persistance de session : permet de créer un
// compte pour un membre du personnel SANS déconnecter le superviseur en cours
// (signUp() bascule sinon automatiquement la session sur le nouveau compte).
function makeTempAuthClient(){
...
function nameSlug(name){
  return stationSlug(name) || 'personne';
}

async function initGerant(){
  hideAllRoots();
  const root = document.getElementById('gerantRoot');
  if(root) root.classList.remove('hidden');

  const setupCard = document.getElementById('gerStationSetupCard');
  const mainContent = document.getElementById('gerMainContent');
  ...
  document.getElementById('gerStationName').textContent = profileInfo.station_name;
  document.getElementById('gerStationLine').textContent = 'Gérant — ' + profileInfo.station_name;

  const label = document.getElementById('gerStaffStationLabel');
  if(label) label.textContent = profileInfo.station_name;

  const qrImg = document.getElementById('stationQrImg');
  if(qrImg){
    qrImg.src = 'https://api.qrserver.com/v1/create-qr-code/?size=260x260&data=' + encodeURIComponent(profileInfo.station_name);
  }

  document.getElementById('staffAddMsg').textContent = '';

  await renderStaffListManage();
  await renderQuartsConfigForm();

  const depDate = document.getElementById('depouillementDate');
  if(depDate && !depDate.value) depDate.value = todayDate();
  await loadDepouillement();
}

async function saveGerantStation(){
  const val = document.getElementById('gerStationInput').value.trim();
  const msg = document.getElementById('gerStationSetupMsg');
  ...
  try{
    const { error } = await sb.from('profiles').upsert({ id: currentUser.id, station_name: val, role: 'gerant' });
    if(error) throw error;
...
// ============================================================
// DÉPOUILLEMENT — tableau central des versements de la station
// ============================================================

let depouillementCache = []; // [{pompisteId, pompisteName, shiftDate, versement}]
let depouillementStationPompistes = [];

function originLabel(origin){
  return {
    'pompiste': 'Pompiste',
    'gerant': 'Ajouté par le gérant',
    'superviseur': 'Ajouté par le superviseur',
    'chef-de-piste': 'Ajouté par le chef de piste'
  }[origin] || 'Pompiste';
}

function currentUserOrigin(){
  const r = String(profileInfo.role || '').toLowerCase();
  if(r === 'gerant' || r === 'gérant') return 'gerant';
  if(r === 'superviseur') return 'superviseur';
  return 'chef-de-piste';
}

async function loadDepouillement(){
  const dateInput = document.getElementById('depouillementDate');
  if(dateInput && !dateInput.value) dateInput.value = todayDate();
  const dateStr = dateInput?.value || todayDate();

  const listBox = document.getElementById('depouillementList');
  if(listBox) listBox.innerHTML = '<div class="empty">Chargement…</div>';
  ...
  try{
    ...
    depouillementStationPompistes = pompistes || [];

    const select = document.getElementById('coffreProfileSelect');
    if(select){
      select.innerHTML = depouillementStationPompistes.map(p => `<option value="${p.id}">${escapeHtml(p.full_name || 'Sans nom')}</option>`).join('') || '<option value="">Aucun pompiste</option>';
    }

    depouillementCache = [];

    for(const p of depouillementStationPompistes){
      const { data: row } = await sb.from('shifts').select('data').eq('user_id', p.id).eq('shift_date', dateStr).maybeSingle();
      if(!row || !row.data) continue;

      (row.data.versements || []).forEach(v => {
        depouillementCache.push({ pompisteId: p.id, pompisteName: p.full_name || 'Sans nom', shiftDate: dateStr, versement: v });
      });
    }

    renderDepouillementStats();
    renderDepouillementList();
  }catch(e){
    if(listBox) listBox.innerHTML = '<div class="empty">Erreur : ' + escapeHtml(e.message) + '</div>';
  }
}

function renderDepouillementStats(){
  const box = document.getElementById('depouillementStats');
  if(!box) return;

  const total = depouillementCache.length;
  const verifies = depouillementCache.filter(e => e.versement.verified).length;
  const nonVerifies = total - verifies;
  const anomalies = depouillementCache.filter(e => e.versement.verified && num(e.versement.ecart) !== 0).length;

  const montantDeclare = depouillementCache.reduce((s, e) => s + num(e.versement.montant), 0);
  const montantVerifie = depouillementCache.filter(e => e.versement.verified).reduce((s, e) => s + num(e.versement.montantReel ?? e.versement.montant), 0);
  const ecartTotal = depouillementCache.filter(e => e.versement.verified).reduce((s, e) => s + num(e.versement.ecart || 0), 0);

  box.innerHTML = `
    <div class="row"><span>Versements</span><span>${total}</span></div>
    <div class="row"><span>Vérifiés</span><span>${verifies}</span></div>
    <div class="row"><span>Non vérifiés</span><span>${nonVerifies}</span></div>
    <div class="row"><span>Anomalies</span><span>${anomalies}</span></div>
    <div class="row total"><span>Montant déclaré</span><span>${fmt(montantDeclare)} F</span></div>
    <div class="row total"><span>Montant vérifié</span><span>${fmt(montantVerifie)} F</span></div>
    <div class="row total"><span>Écart cumulé</span><span style="color:${ecartTotal < 0 ? 'var(--bad)' : 'var(--good)'}">${ecartTotal > 0 ? '+' : ''}${fmt(ecartTotal)} F</span></div>
  `;
}

function renderDepouillementList(){
  const box = document.getElementById('depouillementList');
  if(!box) return;

  if(!depouillementCache.length){
    box.innerHTML = '<div class="empty">Aucun versement pour cette date</div>';
    return;
  }

  box.innerHTML = depouillementCache.map((entry, idx) => {
    const v = entry.versement;
    const type = VTYPES.find(t => t.key === v.type);

    let statutBadge = '<span class="badge" style="color:var(--muted)">⏳ Non vérifié</span>';
    if(v.verified){
      statutBadge = num(v.ecart) === 0
        ? '<span class="badge good">✓ Conforme</span>'
        : `<span class="badge" style="color:${num(v.ecart) < 0 ? 'var(--bad)' : 'var(--amber)'}">${num(v.ecart) < 0 ? '⚠ Manquant' : '⚠ Excédent'} ${num(v.ecart) > 0 ? '+' : ''}${fmt(v.ecart)} F</span>`;
    }

    const origin = v.origin && v.origin !== 'pompiste'
      ? `<div class="s" style="color:var(--amber)">${escapeHtml(originLabel(v.origin))}</div>` : '';

    return `
      <div class="list-item">
        <span class="ic">${type ? type.ic : '💰'}</span>
        <div class="info">
          <div class="t">${escapeHtml(entry.pompisteName)} — ${escapeHtml(type ? type.label : v.type)}</div>
          <div class="s">${escapeHtml(v.time || '')}${v.reference ? ' · Réf. ' + escapeHtml(v.reference) : ''}</div>
          ${origin}
          <div class="s">${statutBadge}</div>
        </div>
        <span class="amt">${fmt(v.montant)} F</span>
        ${!v.verified ? `<button class="del" style="color:var(--good)" onclick="toggleVerifyVersement(${idx})" title="Marquer conforme">✓</button>` : ''}
        <button class="del" style="color:var(--amber)" onclick="correctVersement(${idx})" title="Corriger le montant">✏️</button>
      </div>
    `;
  }).join('');
}

async function persistDepouillementEntry(entry){
  try{
    const { data: row, error: readErr } = await sb.from('shifts').select('data').eq('user_id', entry.pompisteId).eq('shift_date', entry.shiftDate).maybeSingle();
    if(readErr || !row) throw new Error('Service introuvable.');

    const versements = row.data.versements || [];
    const i = versements.findIndex(v => v.id === entry.versement.id);
    if(i === -1) throw new Error('Versement introuvable.');

    versements[i] = entry.versement;
    const updatedData = { ...row.data, versements };

    const { error: updateErr } = await sb.from('shifts').update({ data: updatedData, updated_at: new Date().toISOString() }).eq('user_id', entry.pompisteId).eq('shift_date', entry.shiftDate);
...
async function toggleVerifyVersement(idx){
  const entry = depouillementCache[idx];
  if(!entry) return;

  entry.versement.verified = true;
  entry.versement.montantReel = entry.versement.montant;
  entry.versement.ecart = 0;
  entry.versement.verifiedBy = profileInfo.full_name || 'Responsable';
  entry.versement.verifiedAt = new Date().toISOString();

  const ok = await persistDepouillementEntry(entry);
  if(ok){
    renderDepouillementStats();
    renderDepouillementList();
  }
...
async function correctVersement(idx){
  const entry = depouillementCache[idx];
  if(!entry) return;

  const montantReelStr = prompt('Montant réellement compté (FCFA) :', entry.versement.montant);
  if(montantReelStr === null) return;

  const montantReel = num(montantReelStr);
  const ecart = montantReel - num(entry.versement.montant);

  let motif = '';
  if(ecart !== 0){
    motif = prompt('Motif de l\'écart (obligatoire) :') || '';
    if(!motif.trim()){ alert('Un motif est obligatoire en cas d\'écart.'); return; }
  }

  entry.versement.montantDeclareInitial = entry.versement.montantDeclareInitial ?? entry.versement.montant;
  entry.versement.montantReel = montantReel;
  entry.versement.ecart = ecart;
  entry.versement.motifEcart = motif;
  entry.versement.verified = true;
  entry.versement.verifiedBy = profileInfo.full_name || 'Responsable';
  entry.versement.verifiedAt = new Date().toISOString();
...
async function addCoffreVersement(){
  const pompisteId = document.getElementById('coffreProfileSelect').value;
  const type = document.getElementById('coffreType').value;
  const montant = num(document.getElementById('coffreMontant').value);
  const reference = document.getElementById('coffreReference').value.trim();
  const observation = document.getElementById('coffreObservation').value.trim();
  const dateStr = document.getElementById('depouillementDate')?.value || todayDate();
  const msg = document.getElementById('coffreMsg');

  if(!pompisteId){ alert('Choisissez un pompiste.'); return; }
  if(montant <= 0){ alert('Indiquez un montant valide.'); return; }

  // Protection anti-doublon simple : même pompiste, même montant, même type, même jour
  const duplicate = depouillementCache.find(e =>
    e.pompisteId === pompisteId && e.versement.type === type && num(e.versement.montant) === montant
  );

  if(duplicate && !confirm('Un versement similaire existe déjà pour ce pompiste ce jour-là (même montant et même type). Ajouter quand même ?')){
    return;
  }

  try{
    const { data: row, error: readErr } = await sb.from('shifts').select('data').eq('user_id', pompisteId).eq('shift_date', dateStr).maybeSingle();
    if(readErr || !row || !row.data) throw new Error("Ce pompiste n'a pas de service enregistré à cette date.");
    ...
    const newVersement = {
      ...
      note: observation,
      time,
      origin: currentUserOrigin(),
      verified: true,
      montantReel: montant,
      ecart: 0,
      verifiedBy: profileInfo.full_name || 'Responsable',
      verifiedAt: new Date().toISOString()
    };

    const versements = [...(row.data.versements || []), newVersement];
    const updatedData = { ...row.data, versements };

    const { error: updateErr } = await sb.from('shifts').update({ data: updatedData, updated_at: new Date().toISOString() }).eq('user_id', pompisteId).eq('shift_date', dateStr);
    if(updateErr) throw updateErr;

    if(msg){ msg.textContent = '✓ Versement du coffre ajouté.'; msg.style.color = 'var(--good)'; }

    document.getElementById('coffreMontant').value = '';
    document.getElementById('coffreReference').value = '';
    document.getElementById('coffreObservation').value = '';

    await loadDepouillement();
  }catch(e){
    if(msg){ msg.textContent = 'Erreur : ' + e.message; msg.style.color = 'var(--bad)'; }
  }
}

async function renderQuartsConfigForm(){
  const existing = stationConfig?.quarts_config;
  const mode = existing?.quarts?.length === 2 ? '2' : '3';

  const modeSelect = document.getElementById('quartsMode');
  if(modeSelect) modeSelect.value = mode;

  const quarts = existing?.quarts || defaultQuartsConfig(mode).quarts;
  renderQuartsFields(quarts);
}

function onQuartsModeChange(){
  const mode = document.getElementById('quartsMode').value;
  renderQuartsFields(defaultQuartsConfig(mode).quarts);
}

function renderQuartsFields(quarts){
  const box = document.getElementById('quartsFieldsBox');
  if(!box) return;

  box.innerHTML = quarts.map((q, i) => `
    <div class="fuel">
      <div class="fuel-head">🕐 <input class="input" style="font-weight:800;border:none;background:none;padding:0" id="quartLabel_${i}" value="${escapeHtml(q.label)}"></div>
      <div class="pump">
        <div><label>Heure de début</label><input class="input" type="time" id="quartStart_${i}" value="${q.start}"></div>
        <div><label>Heure de fin</label><input class="input" type="time" id="quartEnd_${i}" value="${q.end}"></div>
      </div>
    </div>
  `).join('');
}

async function saveQuartsConfig(){
  const mode = document.getElementById('quartsMode').value;
  const count = mode === '2' ? 2 : 3;

  const quarts = [];
  for(let i = 0; i < count; i++){
    const label = document.getElementById('quartLabel_' + i)?.value.trim() || ('Quart ' + (i + 1));
    const start = document.getElementById('quartStart_' + i)?.value || '00:00';
    const end = document.getElementById('quartEnd_' + i)?.value || '00:00';
    quarts.push({ id: 'q' + (i + 1), label, start, end });
  }

  stationConfig.quarts_config = { quarts };

  const ok = await persistStationConfig(profileInfo.station_name);
  const msg = document.getElementById('quartsMsg');

  if(msg){
    msg.textContent = ok ? '✓ Quarts enregistrés.' : "Échec de l'enregistrement.";
    msg.style.color = ok ? 'var(--good)' : 'var(--bad)';
  }
}

async function renderStaffListManage(){
  const box = document.getElementById('staffListManageBox');
  if(!box) return;

  box.innerHTML = '<div class="empty">Chargement…</div>';

  try{
    const { data, error } = await sb.from('profiles').select('id,full_name,role').eq('station_name', profileInfo.station_name);
    if(error) throw error;

    const staff = data || [];

    if(!staff.length){
      box.innerHTML = '<div class="empty">Aucun personnel enregistré pour le moment</div>';
      return;
    }
    ...
    box.innerHTML = staff.map(p => `
      <div class="list-item">
        ...
        <button class="del" onclick="resetStaffPin('${p.id}','${escapeHtml(p.full_name || 'Sans nom').replace(/'/g,"\\'")}')" style="color:var(--amber);font-size:11px;white-space:nowrap">🔑 Réinitialiser</button>
      </div>
    `).join('');
  }catch(e){
...
async function resetStaffPin(profileId, profileName){
  const pin = prompt('Nouveau code PIN pour ' + profileName + ' (4 chiffres) :');
  if(!pin) return;
  ...
  try{
    ...
    const res = await fetch(SUPABASE_URL + '/functions/v1/reset-pin', {
      ...
      body: JSON.stringify({ targetUserId: profileId, newPin: pin.trim() })
    });

    const json = await res.json();
    if(!res.ok) throw new Error(json.error || 'Erreur inconnue.');

    alert('✓ PIN réinitialisé pour ' + profileName + '. Communiquez-lui le nouveau code.');
  }catch(e){
    alert("Échec de la réinitialisation : " + e.message);
  }
}

async function addStaffMember(){
  const name = document.getElementById('staffAddName').value.trim();
  const role = document.getElementById('staffAddRole').value;
  const pin = document.getElementById('staffAddPin').value.trim();
  const msg = document.getElementById('staffAddMsg');

  if(!name){ alert('Indiquez le nom du membre du personnel.'); return; }
  if(!/^\d{4}$/.test(pin)){ alert('Le code PIN doit contenir exactement 4 chiffres.'); return; }

  msg.textContent = 'Création en cours…';
  msg.style.color = 'var(--muted)';

  const fakeEmail = `${nameSlug(name)}.${stationSlug(profileInfo.station_name)}.${Date.now().toString(36).slice(-4)}@mastation-interne.app`;

  try{
    const tempClient = makeTempAuthClient();
    ...
    if(error) throw error;
    if(!data.user) throw new Error("Le compte n'a pas pu être créé (confirmation email probablement activée sur le projet Supabase — désactivez-la dans Authentication > Settings).");

    const { error: profErr } = await sb.from('profiles').upsert({
      id: data.user.id,
      full_name: name,
      email: fakeEmail,
      station_name: profileInfo.station_name,
      role
    });

    if(profErr) throw profErr;

    msg.textContent = '✓ ' + name + ' ajouté(e) au personnel.';
...
// Cherche le service "actuel" d'un pompiste en tenant compte du quart de nuit :
// un service ouvert hier peut toujours être en cours si le quart traverse minuit.
async function findCurrentShiftForUser(userId){
  const dates = [todayDate(), yesterdayDate()];

  const { data, error } = await sb.from('shifts').select('shift_date,data').eq('user_id', userId).in('shift_date', dates);
  if(error || !data || !data.length) return null;

  const open = data.find(r => r.data && r.data.status === 'open');
  if(open) return open;

  return data.slice().sort((a, b) => b.shift_date.localeCompare(a.shift_date))[0];
}
...
async function loadSupervisorPompistes(){
  ...
  try{
    ...
    for(const p of pompistes){
      const shift = await findCurrentShiftForUser(p.id);
      rows.push({ profile: p, shift: shift ? shift.data : null });
    }

    box.innerHTML = rows.map(r => {
      const st = r.shift ? r.shift.status : 'none';
      const label = st === 'open' ? 'Poste ouvert' : st === 'validated' ? '✓ Service validé' : st === 'closed' ? 'Clôturé — à valider' : 'Pas encore commencé';
      const dotClass = st === 'open' ? 'open' : (st === 'closed' || st === 'validated') ? 'closed' : 'none';
...
async function openSupervisorDetail(pompisteId){
  const shift = await findCurrentShiftForUser(pompisteId);
  const { data: profRow } = await sb.from('profiles').select('full_name').eq('id', pompisteId).maybeSingle();

  if(!shift || !shift.data || shift.data.status === 'none'){
    alert("Ce pompiste n'a pas encore ouvert son poste aujourd'hui.");
    return;
  }

  renderSupervisorDay(profRow?.full_name || '—', shift.data, new Date().toLocaleDateString('fr-FR', { weekday:'long', day:'numeric', month:'long' }), pompisteId, shift.shift_date);
...
let currentHistoryPompisteId = null;

async function openSupervisorHistory(pompisteId, pompisteName){
  currentHistoryPompisteId = pompisteId;
  ...
  try{
    ...
    supHistoryCache = (data || []).filter(r => r.data && (r.data.status === 'closed' || r.data.status === 'validated'));
...
function openSupervisorDayDetail(dateStr, pompisteName){
  ...
  renderSupervisorDay(pompisteName, row.data, dObj.toLocaleDateString('fr-FR', { weekday:'long', day:'numeric', month:'long', year:'numeric' }), currentHistoryPompisteId, dateStr);
...
function renderSupervisorDay(pompisteName, d, dateLabel, pompisteId, dateStr){
  const t = shiftTotals(d);
  const ok = t.ecart >= 0;

  const statutLabel = d.status === 'validated'
    ? `✓ Validé${d.validatedBy ? ' par ' + escapeHtml(d.validatedBy) : ''}`
    : d.status === 'closed' ? 'Clôturé — en attente de validation' : 'En cours';

  const statutColor = d.status === 'validated' ? 'var(--good)' : d.status === 'closed' ? 'var(--amber)' : 'var(--muted)';
  ...
  document.getElementById('supDetailInfoBox').innerHTML = `
    ...
    <div class="summary-line"><span>Poste / Îlot</span><span class="v">${escapeHtml(d.posteNumber || '—')} / ${escapeHtml(d.ilotNumber || '—')}</span></div>
    ${d.quartLabel ? `<div class="summary-line"><span>Quart</span><span class="v">${escapeHtml(d.quartLabel)}</span></div>` : ''}
    <div class="summary-line"><span>Statut</span><span class="v" style="color:${statutColor}">${statutLabel}</span></div>
  ...
  const validateBox = document.getElementById('supDetailValidateBox');
  if(validateBox){
    if(d.status === 'closed' && pompisteId && dateStr){
      const safeName = (pompisteName || '').replace(/'/g, "\\'");
      validateBox.innerHTML = `
        <button class="btn big-action" onclick="validateSupervisorShift('${pompisteId}','${dateStr}','${safeName}')">✓ Valider le service</button>
      `;
    }else{
      validateBox.innerHTML = '';
    }
...
async function validateSupervisorShift(pompisteId, dateStr, pompisteName){
  if(!confirm('Valider définitivement le service de ' + pompisteName + ' pour cette journée ?\n\nUne fois validé, le pompiste ne pourra plus le modifier directement.')) return;
  ...
  try{
    ...
    const updatedData = {
      ...
      validatedBy: profileInfo.full_name || 'Responsable'
    };

    const { error: updateErr } = await sb.from('shifts').update({ data: updatedData, updated_at: new Date().toISOString() }).eq('user_id', pompisteId).eq('shift_date', dateStr);
    if(updateErr) throw updateErr;

    alert('✓ Service validé.');

    const dObj = new Date(dateStr + 'T00:00:00');
    const dateLabel = dObj.toLocaleDateString('fr-FR', { weekday:'long', day:'numeric', month:'long', year:'numeric' });

    renderSupervisorDay(pompisteName, updatedData, dateLabel, pompisteId, dateStr);
  }catch(e){
    alert("Échec de la validation : " + e.message + "\n\n(Vérifiez que la politique RLS de validation a bien été appliquée dans Supabase.)");
  }
}
